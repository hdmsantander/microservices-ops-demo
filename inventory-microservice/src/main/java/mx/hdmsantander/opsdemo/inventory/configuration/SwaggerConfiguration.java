...
public class SwaggerConfiguration {
	...
	public Docket externalApi() {
		return new Docket(DocumentationType.SWAGGER_2).groupName("Main").select()
				.apis(RequestHandlerSelectors.basePackage("mx.hdmsantander.opsdemo.inventory.controller"))
				.paths(PathSelectors.ant("/v1/**")).build().apiInfo(apiInfo());
	}

	private ApiInfo apiInfo() {
		return new ApiInfo("REST API of the inventory microservice", "REST API for the tracing demo", "1.0", "",
